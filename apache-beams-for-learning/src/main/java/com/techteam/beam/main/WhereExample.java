        ...
        PCollection<String> selectPCollection = productPCollection
                ...
                .apply(ParDo.of(new DoFn<Row, String>() {
                    ...
                    public void processElement(ProcessContext c) {
                        Row row = c.element();
                        assert row != null;
                        String productName = Objects.requireNonNull(checkValue(row, 1)).toString();
                        if (productName.toLowerCase().startsWith("rice")
                                || productName.toUpperCase().startsWith("rice")) {
